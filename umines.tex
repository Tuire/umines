...
Information retrieval tasks the users do are different, and it has been shown that there are different types of searching []. We dive into the exploratory search which has its properties that distinguish it from other searching types. We adopt the description of exploratory search by Marchionini: 
...
An exploratory search task is not a well-defined task that the user stops after finding an answer to a question. 
Instead, as the searcher is not after a specific fact, the search process is iterative and the user might return to the task several times and in several sessions.
The time gap between the sessions means the user is easily distracted or lead to different paths, since they have lost the 'flow' they had in the previous search session []. 
Having lost the flow, their (motivation is weak, easily distracted, open to new ideas) and the overall search goal may get redefined.
The nature of an exploratory search is (hard-to-define). Supporting such a process is challenging, because the tactics and preferences vary.
Although the evaluation of exploratory search user interfaces has been proven to be difficult [], some design guidelines have been proposed and some solutions have been found to help users in their exploratory search tasks.

To this (literature review/lecture/essay) we chose three solutions that are seen in search systems in general, but these solutions can be enhanced further by (taking advantage of) user modeling. 
The chosen solutions were (chosen), because 1) they take place in different stages of the search process, 2) their use of user modeling differs, and 3) the reward the user (gets is different).
While presenting the solutions, these (differences) are explained. In addition, we describe how an exploratory search process benefits from use of user modeling in these three solutions.
